class PlayController < ApplicationController
  ...
  def results
    ...
    @score = session[:score]
    @counter = session[:counter].to_i
    @maximumarray= Answer.where(:tag => params[:question_id].to_s).group("count(content)").order("count(content) DESC").to_a
    @maximum= Answer.where(:tag => params[:question_id].to_s).group("count(content)").order("count(content) DESC").first
