class PlayController < ApplicationController
  ...
  def results
    ...
    @score = session[:score]
    @counter = session[:counter].to_i
    @maximumarray= Answer.select("DISTINCT ON (answer.content) * ").where(:tag => params[:question_id].to_s).group("answers.id", "content").order("count(content) DESC").to_a
    @maximum= Answer.select("DISTINCT ON (answer.content) * ").where(:tag => params[:question_id].to_s).group("answers.id", "content").order("count(content) DESC").first
